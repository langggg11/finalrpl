...
const DEMO_AKUN_ASESI = [
  ...
  // Tambah akun lain di sini jika perlu:
  // {
  //   id: "asesi-dummy-2",
  //   email: "dummy2.asesi@stis.ac.id",
  //   nama: "Dummy Asesi 2",
  //   role: "ASESI",
  //   nim: "000000002",
  //   skemaId: "DS",
...
export async function mockGetProgressAsesi(asesiId) {
  ...
  if (!progress) {
    ...
    // Deteksi akun demo / khusus — mulai dari nol dan wajib lakukan pra-asesmen dulu
    const isDemoAccount = DEMO_AKUN_ASESI.some(d => d.id === user?.id || d.email === user?.email);

    const completedIds = isDemoAccount ? new Set() : new Set(unitsInSkema.slice(0, 3).map((u) => u.id));
    const initialFase = isDemoAccount ? "PRA_ASESMEN" : "PEMBELAJARAN";
    const initialStatusPraAsesmen = isDemoAccount ? "BELUM" : "BELUM";
    ...
    progress = { 
      ...
      statusPraAsesmen: initialStatusPraAsesmen, 
      tryoutSelesai: false, 
